...
while not finished:
    ...
    if keys[pg.K_n]:
        size /= 1.1
        clone = pg.transform.rotozoom(img, angle, size)
    if keys[pg.K_h]:  # отражение по горизонтали
        clone = pg.transform.flip(img, True, False)  # flip(что_вращать, по_оси_х, по_оси_у) - отражает изображение по одной из осей
    if keys[pg.K_v]:  # отражение по вертикали
        clone = pg.transform.flip(img, False, True)

    clone_rect = clone.get_rect()  # получаю квадрат изображения
    clone_rect.center = W // 2, H // 2  # центрирую картинку посередине
